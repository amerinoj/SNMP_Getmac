 ...
 for DEVICE in $LIST ; do {
        ...
        #Check the host response
         if [[ $HOST == "" ]]; then
				...
				Vlan_Id_Table=$(snmpwalk -v3  -u $USER -a $AUTHP -A $PASSWORD -x $PRIVP -X $PASSWORD -l authPriv $DEVICE \
				...
				########## GET MAC TABLE SECCTION #######################
   				...
   				if [[ -n $Vlan_Id_Table ]]; then #SI Vlan_Id_Table no  es null
					...
					Mac_Index_Table=$(snmpwalk -v3  -u $USER -a $AUTHP -A $PASSWORD -x $PRIVP -X $PASSWORD -l authPriv $DEVICE  \
					...
					#Extreme
						D_TYPE=2
						Index_Id_Table=$(snmpwalk -v3  -u $USER -a $AUTHP -A $PASSWORD -x $PRIVP -X $PASSWORD -l authPriv $DEVICE \
						...
						if [[ -n $Index_Id_Table ]]; then #SI Index_Id_Table no es null
							Mac_Exteme=$(snmpwalk -v3  -u $USER -a $AUTHP -A $PASSWORD -x $PRIVP -X $PASSWORD -l authPriv $DEVICE \
